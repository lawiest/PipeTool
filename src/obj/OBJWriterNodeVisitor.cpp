...
unsigned int OBJWriterNodeVisitor::OBJMaterial::s_objmaterial_id = 0;

OBJWriterNodeVisitor::OBJMaterial::OBJMaterial(osg::Material* mat, osg::Texture* tex, bool same) :
    diffuse(1,1,1,1),
...
void OBJWriterNodeVisitor::processStateSet(osg::StateSet* ss)
{
    ...
    if (mat || tex)
    {
        _materialMap.insert(std::make_pair(osg::ref_ptr<osg::StateSet>(ss), OBJMaterial(mat, tex,_bMatSame)));
        _fout << "usemtl " << _materialMap[ss].name << std::endl;
    }
