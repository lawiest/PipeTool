...
class OBJWriterNodeVisitor: public osg::NodeVisitor {
    ...
    public:
        OBJWriterNodeVisitor(std::ostream& fout, const std::string materialFileName = "", bool same = false) :
			...
			_bMatSame(same)
        {
			OBJWriterNodeVisitor::OBJMaterial::s_objmaterial_id = 0;
        ...
        class OBJMaterial {
            public:
                OBJMaterial() {}
                OBJMaterial(osg::Material* mat, osg::Texture* tex, bool same = false);

                osg::Vec4  diffuse, ambient, specular;
                std::string    image;
                std::string name;
				static unsigned int s_objmaterial_id;
