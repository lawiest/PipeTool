...
public:
	...
	/// \brief 取值操作符
	GsString::reference operator[](int _Off);
	/// \brief 取值操作符
	GsString::const_reference operator[](int _Off) const;
...
public:
	...
	/// \brief 从url的查询串?key=value&key1=value1
	void ParseFromUrlQueryString(const char* kvp);
...
public:
	...
	/// \brief 转换为字符串对象
	/// \param maxDigitCount 最大有效数字的个数；最多可以精确maxDigitCount位有效数字
	static GsString ToString(float v, int maxDigitCount = 6);
	/// \brief 转换为字符串对象
	/// \param maxDigitCount 最大有效数字的个数；最多可以精确maxDigitCount位有效数字
	static GsString ToString(double v, int maxDigitCount = 15);
