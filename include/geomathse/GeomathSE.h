...
struct geo_object
{
    ...
    /// \brief 得到内部成员变量的指针
    /// \param mode 可以取值 m_ptr_path_i4,m_ptr_path_s_xy,m_ptr_path_s_z,m_ptr_path_m_subs
    virtual void* m_ptr(int mode) { return 0; }
};
...
struct geo_vector : geo_object
{
    ...
    // 如果第i个元素是指针类型，那么参数ptr将被忽略，返回值将得到元素指针，外部程序再转换为string_t或wstring_t或geo_object*,或geo_blob*
	...
	// 如果第i个元素是指针类型，那么ptr必须是与之匹配的string_t,wstring_t或geo_object*或geo_blob*
...
/// \brief 内存表中支持的blob数据类型
struct geo_blob
{
	long long	m_cs;       //m_data的字节数
    void*		m_data;
};
...
struct geo_cell
{
    ...
    /// \brief 数据的类型 
    /// \details 目前只能是byte,string,wstring,object,blob
	string_t	type;
...
{	
	...
	/// \brief aliasname 别名
	/// \brief 修改结果
	virtual bool modify_aliasname(int columns, const char* aliasname) = 0;
...
/// 将一个geo_clip对象传入geo_algorithm::do_clip函数实现单层瓦片的切割
struct geo_clip
{
	string_t	m_fn_geo;	    //数据的文件名，数据格式必须是g2d
    int         m_level;        //当前层及
    int         m_min_ptn;      //单个瓦片最少的点个数，如果某个瓦片点个数小于该值则该瓦片不会产生 
	double	    m_x0, m_y0;		//原点
	double	    m_dx, m_dy;		//0级瓦片在 x,y轴上的步长,瓦片的长宽
	double	    m_tx, m_ty;		//瓦片外扩比例,一般是1/16

	//当切片程序切完一个瓦片后会调用此函数
	//用户通过此函数实现数据的入库
	virtual void on_clip(int x,int y,geo_reader* R) = 0;
};
...
struct geo_factory : geo_object
{
    ...
    //scrip="point_zm" 返回point_zm的trans 此trans用来恢复path的z值 此trans需要设置 tol参数 和path 参数
	...
	/// \brief 设置环境变量(跨动态库设置环境变量好像不起效果,所以添加这个函数)
	virtual void setenv(string_t name,string_t value) = 0;

	/// \brief 得到全局的进度值进度范围[0,1]
	/// 某些费时的函数内部会使用进度值,外部线程可以通过这个函数得到进度
	virtual double progress_get() = 0;

    /// \brief 三角网变换
    /// \param cdim 坐标维数,只能取值2,3，该值指定xy,uv中的坐标维数
    /// \param num 点个数
    /// \param xy 源点
    /// \param uv 目标点
    /// \return 三角网变换对象，该对象能够将一个path对象进行坐标转换，转换的目的是将xy微调到uv
    virtual geo_trans* tin_map(int cdim, int num, const double* xy, const double* uv,const double* env) = 0;

    /// \brief 构造一个path_3对象
    /// \param  npt 点个数 
    /// \param  xyz xyz数据
    /// \param  uv uv数据
    /// \param  vcs是数据的字节数，只能传递4或8，4表示float类型8表示double类型
    /// \param  isref 指定，内部的xyz和uv是否引用外面传入的指针
    virtual geo_object* new_path_3(int npt, const void* xyz,const void* uv,int vcs, bool isref) = 0;


    /// \brief 构造一个path_t对象
    /// \param  ntin 三角形个数 
    /// \param  tin 三角形数据，每三个整数表示一个三角形，整数为p3A中点的下标
    /// \param  vcs是数据的字节数，只能传递1,2,4，1表示uint8,2表示uint16，4表示uint32
    /// \param  isref 指定，内部的tin是否引用外面传入的指针
    virtual geo_object* new_path_t(geo_object* p3A,int ntin, const void* tin, int vcs, bool isref) = 0;
};
...
struct geo_algorithm : geo_object
{
    ...
    /// \param xy 点坐标
    /// \return 返回xy在p上的参数
    virtual double xy2t(geo_object* p,const double* xy)=0;
    ...
    // 如果mode=spr_query_area那么格式为[area,a->i4(0),…,b->i4(0),…)
    ...
    virtual int top_reserve_simplify(int n,geo_object** pp,double tol,double* env)=0;
    ...
    /// \brief 构建三角网
    /// \param mode 构网方式 目前支持 "delaunay","polygon" 两种参数
    /// \param npt 输入的点个数
    /// \param xy 输入的点坐标
    /// \param tins 输出的三角形下标 每三个int表示一个三角形,tins[i]为xy中点的下标
    /// \return 三角形的个数ntin
    /// \detail tins的空间由外部分配,应该预留6*npt的空间
    ///         mode为"delaunay"的时候构造狄罗尼三角网
    ///         mode为"polygon"的时候构造面内的三角网
    virtual int build_tins(string_t mode,int npt,const double* xy,int* tins)=0;

    /// \brief 线段构建polyline
    /// \param mode 构线方式 0不关心方向,1关心方向
    /// \param nline 输入的线段个数
    /// \param xy 输入的线段坐标
    /// \return 构出的path
    virtual geo_object* build_lines(int nline,const double* xy,int mode)=0;

    /// \brief 三角网过滤
    /// \param mode 过滤方式 目前支持 "env" 参数
    /// \param para mode等于"env"的时候是外接矩形[x0,y0,x1,y1)
    /// \param cdim 输入点的坐标维数
    /// \param xy 输入输出的点的坐标
    /// \param ntin 输入三角形个数
    /// \param tins 输入输出的三角形下标
    /// \return 三角形的个数ntin
    virtual int select_tins(string_t mode,void* para,int cdim,double* xy,int ntin,int* tins)=0;
	
	/// \brief 瓦片切割
	/// \param cliper 切割的参数
	/// \param env 要切割的瓦片范围，如果env为0程序会便利cliper中的文件得到整个范围
	//	如果用户给定了env则env外的瓦片将不会被on_clip
	virtual void clip_cut(geo_clip* cliper,double* env)=0;

	/// \brief 瓦片缩并
	/// \param source 源文件的文件名(必须是g2d文件)
	/// \param target 目标文件的文件名
	/// \param tol_dis 距离容差 内部使用这个参数对数据进行保拓扑压缩
	/// \param tol_area 面积容差 内部将面积小于tol_area的对象缩并到其附近的大面
	virtual void clip_union(string_t source,string_t target,double tol_dis,double tol_area) = 0;
    
    /// \brief 拓扑一致性
    /// \param fn_in 输入的g2d文件名
    /// \param fn_out 输出的g2d文件名
    /// \param tol 容差
    virtual void consistent(string_t fn_in, string_t fn_out, double tol) = 0;

	/// \brief 点的坐标聚合
	/// \param mode 聚合模式(0弱聚合,1强聚合)
	/// \param cdim 坐标维数(函数只聚合xy两维的值)
	/// \param npt 点的个数
	/// \param xy 点的坐标
	/// \param tol 聚合容差
	virtual void point_agg(int mode, int cdim,int npt,double* xy,double tol) = 0;

	/// \brief 点的坐标聚合
	/// \param mode 聚合模式(0弱聚合,1强聚合)
	/// \param p 聚合对象
	/// \param tol 聚合容差
	virtual void point_agg(int mode, geo_object* p, double tol) = 0;

    /// \brief 长度转为参数
    /// \param len 距离起点的长度
    /// \param pre_lt 前一次调用的(len,t)，利用该值可以加快执行速度
    /// \return 参数
    virtual double length2t(geo_object* p, double len,double* pre_lt) = 0;
    
	/// \brief 瓦片切割
	/// \param cliper 切割的参数
	/// \param env 要切割的瓦片范围，如果env为0程序会便利cliper中的文件得到整个范围
	//	如果用户给定了env则env外的瓦片将不会被on_clip
	/// virtual void clip_cut2(geo_clip2* cliper, double* env) = 0;

    /// \brief 插入n个点
    /// \param s 几何对象
    /// \param T s上的参数
    /// \param xy 点坐标
    virtual void insert_at(geo_object* s, int n, const double* T, const double* xy) = 0;
};
...
/// \brief path_s点个数
const int i4_path_ptn = -3;

/// \brief path_m子对象个数
const int i4_path_subn = -4;

/// \brief path_t的三角形个数
const int i4_path_tin = -5;

/// \brief path内部的数据的字节数
const int i4_path_vcs = -6;

/// \brief path内部的数据的字节数
const int i4_path_has_arc = -7;

const int m_ptr_path_i4 = 1;
const int m_ptr_path_s_xy = 2;
const int m_ptr_path_s_z = 3;
const int m_ptr_path_m_subs = 4;
const int m_ptr_spi_path= 5;
const int m_ptr_path_3_xyz = 6;
const int m_ptr_path_3_uv = 7;
const int m_ptr_path_t_tin = 8;
const int m_ptr_path_t_p3A = 9;
...
/// \brief 得到几何对象的简化对象
const int prop_geo_simplify2 = 14;
...
/// \brief gmodify_dauglas2 (gmodify的mode类型)
/// \details gmodify的另一个参数para为距离
const int gmodify_dauglas2 = 6;
...
/// \brief 容错点 在容差范围内但不精确相同 (点与点)(gcheck_2的mode类型)
const int gc2_pt_tol_x_0 = 0x08;

/// \brief 容错点 在容差范围内但不精确相同 (点与线)(gcheck_2的mode类型)
const int gc2_pt_tol_x_1 = 0x10;

/// \brief 相交的节点 一个必须是节点 (gcheck_2的mode类型)
const int gc2_pt_nodal = 0x20;		

/// \brief 相穿点 (gcheck_2的mode类型)
const int gc2_pt_cross = 0x40;		
...
const int spr_query_area = 0x40000000;
