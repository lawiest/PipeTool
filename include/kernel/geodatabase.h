...
{
	/// \brief 数据库是否支持事务
	eSupportTransaction = 1,

	/// \brief 数据库是否支持矢量地物类的存储
	eSupportStoreFeatureClass = 1 << 1,

	/// \brief 数据库是否支持普通二维表的存储
	eSupportStoreRowClass = 1 << 2,

	/// \brief 数据库是否支持瓦片类的存储
	eSupportStoreTileClass = 1 << 3,

	/// \brief 数据库是数据集目录
	eSupportDataRoomFolder = 1 << 4,

	/// \brief 数据库是数据集目录多级递归
	/// \details 数据集目录下是否还可以支持数据集目录
	eSupportRecursiveDataRoomFolder = 1 << 5,

	/// \brief 数据源是否支持栅格数据存储
	eSupportStoreRasterClass = 1 << 6,

	/// \brief 数据源是否支持镶嵌栅格数据存储
	eSupportStoreMosaicRasterClass = 1 << 7,

	/// \brief 是否支持新增属性字段
	eSupportAddField = 1 << 8,

	/// \brief 是否支持删除字段
	eSupportDeleteField = 1 << 9,

	/// \brief 是否支持修改字段
	eSupportModefyField = 1 << 10,

	/// \brief 是否支持插入字段
	eSupportInsertField = 1 << 11,

	/// \brief 数据是否支持删除地物
	eSupportDeleteFeature = 1 << 12,

	/// \brief 数据是否支持修改地物
	eSupportModifyFeature = 1 << 13,

	/// \brief 数据是否支持中间插入地物
	eSupportInsertFeature = 1 << 14,

	/// \brief 是否支持尾部插入
	eSupportAddFeature = 1 << 15,

	/// \brief 数据是否支持属性索引
	eSupportAttributeIndex = 1 << 16,

	/// \brief 是否支持空间索引
	eSupportSpatialIndex = 1 << 17
};
...
enum GsFeatureType
{
	...
	/// \brief 复合地物类，
	/// \details 允许单个地物类中存储的地物几何类型可以点、线、面任意类型
	eCompoundFeature,
...
class GS_API GsMosaicRasterManager :public GsDataRoomExtensionData
{
protected:
	GsMosaicRasterManager();
public:
	virtual ~GsMosaicRasterManager();

	/// \brief 添加Raster
	/// \param pRaster 
	/// \return 
	virtual bool AddRaster(GsRasterClass *pRaster) = 0;
	/// \brief 移除Raster
	/// \param pRaster 
	/// \return 
	virtual bool RemoveRaster(GsRasterClass *pRaster) = 0;

	/// \brief 添加Raster
	/// \param pRasterPath 
	/// \return 
	virtual bool AddRaster(const char* pRasterPath) = 0;
	/// \brief 移除Raster
	/// \param pRasterName 
	/// \return 
	virtual bool RemoveRaster(const char* pRasterName) = 0;
	/// \brief 清空Raster
	/// \return 
	...
	/// \brief 获取相关联的所有栅格
	/// \return 
	virtual UTILITY_NAME::GsVector< Utility::GsSmarterPtr<GsRasterClass>> RasterClasses() = 0;

	/// \brief 获取相关联的所有栅格名称
	/// \return 
	virtual UTILITY_NAME::GsVector<UTILITY_NAME::GsString> RasterClassesName() = 0;

	/// \brief 通过存储的ID查询
	/// \param id 
	/// \return 
	virtual Utility::GsSmarterPtr<GsRasterClass> RasterClass(int id) = 0;
	/// \brief 通过名称查询
	/// \param strName 
	/// \return 
	virtual Utility::GsSmarterPtr<GsRasterClass> RasterClass(const char* strName) = 0;

	/// \brief 获取Dirty的状态。
	/// \return 
	virtual bool Dirty() = 0;
	/// \brief 数据发生变化
	/// \param bool bl True 数据发生了变化，False：已经处理了数据发生的变化.
	/// \param bl 
	virtual void Dirty(bool bl) = 0;
};
/// \brief GsVectorTileStyleExtensionDataPtr
GS_SMARTER_PTR(GsMosaicRasterManager);
...
protected:
	...
	GsDataRoomExtensionDataPtr				m_ptrExtData;
	GsDataRoom();
public:
	virtual ~GsDataRoom();
...
protected: 
	...
	Utility::GsStlMap<Utility::GsString, int> m_mapName2Index;
	GsFeature();
public:
	virtual ~GsFeature();
	...
	virtual int  FindFiledIndex(const Utility::GsString& strName);
};
/// \brief GsFeaturePtr
...
/// \brief 分析瓦片数据封装
class GS_API GsTileBuffer
{
protected:
	UTILITY_NAME::GsGrowByteBuffer m_Buffer;
...
public:
	GsTileBuffer();
	virtual ~GsTileBuffer();

	/// \brief 从瓦片读取数据
	/// \param pTile 
	/// \return 
	bool ReadFromTile(GsTile* pTile);
	/// \brief 写入数据到瓦片
	/// \param pTile 
	/// \return 
	bool WriteToTile(GsTile* pTile);
	...
	/// \param nLen 
	/// \return 
	virtual unsigned char*  TileData(int nLen);

	/// \brief 获取瓦片数据指针
	/// \return 
	virtual unsigned char * TileDataPtr();
	/// \brief 获取瓦片数据长度
	/// \return 
	virtual int TileDataLength();

	/// \brief 设置瓦片数据
	/// \param pData 
	/// \param nLen 
	virtual void TileData(const unsigned char* pData, int nLen);

};


/// \brief 瓦片数据IO
class GS_API GsTileDataIO
{
public:
	virtual ~GsTileDataIO();
	...
	/// \return ,当输入空间数据时返回0标示正常输入一条数据，否则为错误或者输入完成。
	/// \param pData 
	virtual int OnData(GsTileBuffer* pData);
};
...
public:
	...
	/// \brief 创建属性索引
	/// \param strFieldNames 字段名列表
	/// \param bUnique 是否是唯一索引
	/// \param strIndexName 索引名称
	/// \return 
	virtual bool CreateAttributeIndex(Utility::GsVector<Utility::GsString>& strFieldNames, bool bUnique, const char* strIndexName);

	/// \brief 创建属性索引
	/// \param FieldIndexes 字段索引列表
	/// \param bUnique 是否是唯一索引
	/// \param strIndexName 索引名称
	/// \return 
	virtual bool CreateAttributeIndex(Utility::GsVector<int>& FieldIndexes, bool bUnique, const char* strIndexName);

	/// \brief 删除属性索引
	/// \param strIndexName 属性索引名称
	/// \return 
...
public:
	...
	/// \param pFilter 
	/// \return 
	virtual GsFeatureDataIO* Search(GsQueryFilter* pFilter);
...
public:
	...
	/// \brief 设置数据回调IO对象
	/// \param IO 
	virtual void DataIO(GsProxyFeatureClassIO* IO);
	/// \brief 获取数据回调IO对象
	/// \return 
	virtual GsProxyFeatureClassIO* DataIO();
...
/// \brief 代理瓦片类回调
class GS_API GsProxyTileClassIO :public Utility::GsRefObject
{
public:
	virtual ~GsProxyTileClassIO();
	/// \brief 搜索一个瓦片类返回数据IO
	/// \return 
	virtual GsTileDataIO* Search();
	/// \brief 搜索一个瓦片类返回数据IO
	/// \param nStartLevel 
	/// \param nEndLevel 
	/// \return 
	virtual GsTileDataIO* Search(int nStartLevel, int nEndLevel);
	/// \brief 搜索一个瓦片类返回数据IO
	/// \param nLevel 
	/// \param nStartRow 
	/// \param nStartCol 
	...
	/// \param nEndCol 
	/// \return 
	virtual GsTileDataIO* Search(int nLevel, int nStartRow, int nStartCol, int nEndRow, int nEndCol);
};

/// \brief GsProxyTileClassIOPtr
GS_SMARTER_PTR(GsProxyTileClassIO);

/// \brief 用于用户实现将任意数据源实现为瓦片数据
/// \details 通过用户实现简单的回调接口实现读取任意数据的能力
class GS_API GsProxyTileClass :public GsTileClass
{
	GsProxyTileClassIOPtr m_ptrIO;
	GsTileColumnInfo m_Info;
	GsSpatialReferencePtr m_ptrSR;
	GsPyramidPtr m_ptrPyramid;
public:
	/// \brief 
	/// \param name 
	GsProxyTileClass(const char* name);
	virtual ~GsProxyTileClass();

	/// \brief 设置数据回调IO对象
	/// \param IO 
	virtual void DataIO(GsProxyTileClassIO* IO);
	/// \brief 获取数据回调IO对象
	/// \return 
	virtual GsProxyTileClassIO* DataIO();

	/// \brief 创建一个新的Tile对象
	/// \return 返回Tile对象的指针
	virtual GsTilePtr CreateTile();

	/// \brief 检索所有的瓦片
	/// \return 返回瓦片游标
	virtual GsTileCursorPtr Search();

	/// \brief 检索某个级别到某个级别的瓦片
	/// \param nStartLevel 开始的级别
	/// \param nEndLevel 结束的级别
	/// \return 返回瓦片游标
	virtual GsTileCursorPtr Search(int nStartLevel, int nEndLevel);
	...
	/// \param nEndCol 开始列
	/// \return 返回瓦片游标
	virtual GsTileCursorPtr Search(int nLevel, int nStartRow, int nStartCol, int nEndRow, int nEndCol);
	...
	/// \param pPyramid 要修改的金字塔对象
	/// \return 返回修改是否成功
	virtual bool ChangePyramid(GsPyramid* pPyramid);

	/// \brief 获取瓦片基本元信息
	/// \return 返回原信息对象
	virtual GsTileColumnInfo TileColumnInfo();

	/// \brief 设置瓦片基本元信息
	/// \param tileinfo 要修改的瓦片元信息对象
	/// \return 返回是否设置成功
	virtual bool TileColumnInfo(const GsTileColumnInfo& tileinfo);

	/// \brief 获取所有瓦片的数量
	/// \return 返回瓦片总数
	virtual long long TileCount();

	/// \brief 获取某个级别瓦片的数量
	/// \param nStartLevel 开始级别
	/// \param nEndLevel 结束级别
	/// \return 返回该级别瓦片总数
	virtual long long TileCount(int nStartLevel, int nEndLevel);
	...
	/// \param nEndCol 开始列
	/// \return 返回该级别瓦片总数
	virtual long long TileCount(int nLevel, int nStartRow, int nStartCol, int nEndRow, int nEndCol);
...
/// \brief GsProxyTileClassPtr
GS_SMARTER_PTR(GsProxyTileClass);
...
public:
	...
	/// \brief 获取数据库的能力集
	/// \return 返回数据库能力枚举的按位或结果
	virtual long long CapbilitySet() = 0;
};
