...
class GsKernel
{
	...
	static void RegisterElement(GsKernelModule module)
	{
		...
		if (module & eKernelDebug) GS_D << "Register GsBlockElement";
		REG_CLASS_CREATE(GsBlockElement);
		REG_CLASS_CREATE_ALIAS(GsBlockElement, "BlockElement");

		if (module & eKernelDebug) GS_D << "Register GsMultiElement";
		REG_CLASS_CREATE(GsMultiElement);
		REG_CLASS_CREATE_ALIAS(GsMultiElement, "MultiElement");
