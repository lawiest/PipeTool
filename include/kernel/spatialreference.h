...
class GS_API GsSpatialReference:public Utility::GsRefObject
{
	...
	/// \brief WKT串纠正
	/// \brief rectify是否开启全局空间参考配置纠正
	/// \param rectify 
	void WKTCheck(bool rectify = false);
	/// \brief 全局空间参考配置纠正
	void CheckAll();
public:
	~GsSpatialReference();
...
public:
	...
	/// \brief 从已知空间参考名称构造
	/// \param srs 
	GsGeodesic(GsWellKnownSpatialReference srs);
...
/// \ brief 最小二乘求解仿射变换参数
/// \ details 通过最小二乘求解仿射变换矩阵
/// \ details 支持2,3维的仿射变换矩阵求解
class GS_API GsLeastSquaresAffineParameter
{
	/// \ brief 矩阵维度
	/// \ details m_dim = 2 时，按仿射变换公式构造矩阵方程
	...
	/// \ details [m21,m22,0]	  
	/// \ details [dx ,dy ,1]
	/// \ details m_dim = 3 时，按空间变换公式构造矩阵方程
	/// \ details X_new = a1 * X + a2*Y + a3 * Z + dX;
	/// \ details Y_new = b1 * X + b2*Y + b3 * Z + dY;
	/// \ details Z_new = c1 * X + c2*Y + c3 * Z + dZ;
	/// \ details 空间变换矩阵常规表达
	/// \ details [a1 ,b1 ,c1 ,0]     
	/// \ details [a2 ,b2 ,c2 ,0]	  
	/// \ details [a3 ,b3 ,c3 ,0]
	...
	int m_dim;
	/// \ brief 传入同名点个数
	/// \ details 如果m_dim为2，则m_Count>=3时，开始计算
	/// \ details 如果m_dim为3，则m_Count>=4时，开始计算
	int m_Count;

	double m_A[169];
	double m_X[13];
	Utility::GsVector<double> m_Param;
public:
	/// \brief 从矩阵维度构造
	/// \param dim 
	GsLeastSquaresAffineParameter(int dim);
	/// \brief 析构函数
	~GsLeastSquaresAffineParameter();
	///  \开始计算
	/// \brief 
	void Reset();
	/// \brief 获取矩阵维度
	...
	/// \param x2 
	/// \param y2 
	void AddPoint(double x1, double y1, double x2, double y2);
	/// \brief 加入三维同名点
	/// \param x1 
	/// \param y1 
	/// \param z1 
	...
	/// \param y2 
	/// \param z2 
	void AddPoint(double x1, double y1, double z1, double x2, double y2, double z2);

	/// \brief 返回仿射变换矩阵
	/// \return 
	const Utility::GsVector<double>& Matrix();
};
enum GsAffineTransformationType
{
	/// \brief 平移变换
	eTranslate,
	/// \brief 错切变换
...
public:
	/// \brief 
	GsAffineCoordinateTransformation() {}
	using GsCoordinateTransformation::Transformation;
	...
	/// \brief 根据仿射变换类型和仿射变换参数构造
	/// \brief 如果type为eTranslate，则做平移变换，paramX为X方向平移量，paramY为Y方向平移量
	/// \brief 如果type为eShear，则做错切变换，paramX为横向剪切距离，paramY为纵向剪切距离
	/// \brief 如果type为eMirror，则做镜像变换，paramX为横向对称轴，paramY为纵向对称轴
	/// \param paramX 横向距离
	/// \param paramY 纵向距离
	/// \param type 变换的类型
	GsAffineCoordinateTransformation(double paramX, double paramY,GsAffineTransformationType type);

	/// \brief 根据旋转点和旋转参数构造
	/// \param rotationAngle为旋转角,单位角度
	/// \param ox为旋转中心点d的坐标值x
	/// \param oy为旋转中心点d的坐标值y
...
/// \brief 三维仿射变换
class GS_API GsAffineCoordinateTransformation3D : public GsAffineCoordinateTransformation
{
	double m_Matrix[12];
public:
	using GsCoordinateTransformation::Transformation;
	/// \brief 根据源和目标同名点组构成仿射变换参数
	/// \details 最低需要4个点的坐标,即nLen为12
	/// \param pSourcePoint 
	/// \param pTargetPoint 
	/// \param nLen 
	GsAffineCoordinateTransformation3D(const double* pSourcePoint, const double* pTargetPoint, int nLen = 12);

	/// \brief 根据访射变换矩阵构造
	/// \details 访射变换矩阵为12个元素
	/// \param pMatirx 
	GsAffineCoordinateTransformation3D(const double* pMatirx);
	/// \brief 设置同名点参数
	/// \param pSourcePoint 
	/// \param pTargetPoint 
	/// \param nLen 
	/// \return 
	bool Parameter(const double* pSourcePoint, const double* pTargetPoint, int nLen = 12);
...
/// \brief 站心坐标转换类型
enum GsSiteCentricTransformType
{
	/// \ brief 地心到站心
	eGeocentricToSiteCentric,
	/// \ brief 站心到地心
	eSiteCentricToGeocentric,
	/// \ brief 地理到站心
	eGeodeticToSiteCentric,
	/// \ brief 站心到地理
	eSiteCentricToGeodetic,
};
/// \brief 站心坐标转换
class GS_API GsSiteCentricCoordinateTransformation :public GsCoordinateTransformation
{
	/// \ brief 空间参考
	GsSpatialReferencePtr m_SR;
	/// \ brief 转换类型
	GsSiteCentricTransformType m_Type; 
public:
	using GsCoordinateTransformation::Transformation;
	/// \brief 从站心原点(e=0,n=0,u=0)对应的经纬度坐标构造
	/// \param x 
	/// \param y 
	/// \param z 
	/// \param sr 
	/// \param type 
	GsSiteCentricCoordinateTransformation(double x, double y, double z=0, 
		GsSpatialReference* sr = NULL, GsSiteCentricTransformType type= eGeodeticToSiteCentric);
	
	/// \brief 设置站心转换类型
	/// \param type 
	void Type(GsSiteCentricTransformType type);
	/// \brief 获取站心转换类型
	/// \return 
	GsSiteCentricTransformType Type();
	...
	virtual bool Transformation(double* pX, double *pY, double *pZ, int nPointCount, int nPointOff);
protected:
	/// \ brief 初始转换参数，实际上12个参数，为了矩阵求逆构成方阵，补一列0,0,0,1
	double m_Matrix[16];
	/// \ brief 瞬时转换参数
	double m_TempMatrix[16];
	/// \ brief 组合坐标转换数组
	UTILITY_NAME::GsVector<GsCoordinateTransformationPtr> m_CT;
};
/// \brief GsSiteCentricCoordinateTransformationPtr
GS_SMARTER_PTR(GsSiteCentricCoordinateTransformation);
...
public:
	...
	static GsCoordinateTransformationPtr CreatePlane7PatameterCoordinateTransformation(GsSpatialReference* pSource, 
	...
	/// \param Mp为计算得中误差残差值,按照规范Mp值不超过比例尺分母的万分之一(0.0001*MapScale)
	static void ComputeBursa7Parameter(GsSpatialReference* pSource, GsSpatialReference* pTarget,
		int n, int dim, const double* Source, const double* Target, double* Param, double& Mp);
	/// \brief 
	/// \param Source 源空间参考
	/// \param Target 目标空间参考
	/// \param nPointCount 
	...
	/// \param pSource 源空间参考
	/// \param pTarget 目标空间参考
	/// \param P7Param 为长度为7的数组,依次存入Bursa7参数(3个平移参数dx,dy,dz,单位为米,3个旋转参数anx,any,anz,单位为秒;一个尺度参数,单位为ppm)
	/// \return 
